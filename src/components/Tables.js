...
import { pageVisits, pageTraffic, pageRanking, orderMockupJsons } from "../data/tables";
...
export const RecentOrdersTable = () => {
    ...
    const TableRow = (props) => {
        ...
        switch (confirmation_level) {
            case 0: confirmation_badge = <Badge bg="danger" className="me-1">결제 완료</Badge>; break;
            case 1: confirmation_badge = <Badge bg="warning" className="me-1">제조중</Badge>; break;
            case 2: confirmation_badge = <Badge bg="info" className="me-1">픽업 대기중</Badge>; break;
            case 3: confirmation_badge = <Badge bg="success" className="me-1">주문 완료</Badge>; break;
        }
            ...
            <React.Fragment>
                <Modal size="lg" as={Modal.Dialog} centered show={showDefault} onHide={() => setShowDefault(false)}>
                    ...
                    <Modal.Body className="pb-0">
                        <h6>승인일자: {approval_date}</h6>
                        <h6>승인번호: {approval_number}</h6>
                    </Modal.Body>
                    <Modal.Body>
                        <Table hover bordered responsive className="align-items-center table-flush">
                            ...
                            <tbody className="text-center" style={{'height': '100'}}>
                                {menus.map(menu => {
                                    return (
                                        <tr>
                                            <th>{menu.name}</th>
                                            <th>{menu.total_price}</th>
                                            <th>{menu.options.map(option => {
                                                if (option.hasOwnProperty('value')) {
                                                    if (option.value === true || option.value === false) {
                                                        return <div>{option.name}: {option.value ? <FontAwesomeIcon icon={faCheckCircle} /> : <FontAwesomeIcon icon={faTimes} />}</div>
                                                    }
                                                    return <div>{option.name}: {option.value.toString()}</div>
                                                }
                                                else if (option.hasOwnProperty('quantity'))
                                                    return <div>{option.name}: {option.quantity}</div>
                                            })}</th>
                                        </tr>
                                    )
                                })}
    ...
    return (
        <Card border="light" className="shadow-sm">
            ...
            <Table hover bordered responsive className="align-items-center table-flush">
                ...
                <tbody>
                {orderMockupJsons.map(pv => <TableRow key={`order-${pv._id}`} {...pv} />)}
                </tbody>
            </Table>
        </Card>
    );
